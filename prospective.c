//implement prospective projection in 3d object using graphics.h
#include <stdio.h>
#include <graphics.h>
#include <math.h>
...
void prospectiveProjection(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int x5, int y5, int x6, int y6, int x7, int y7, int x8, int y8, int d)
{
    int x1_new = x1 * d / (d + y1);
    int y1_new = y1 * d / (d + y1);
    int x2_new = x2 * d / (d + y2);
    int y2_new = y2 * d / (d + y2);
    int x3_new = x3 * d / (d + y3);
    int y3_new = y3 * d / (d + y3);
    int x4_new = x4 * d / (d + y4);
    int y4_new = y4 * d / (d + y4);
    int x5_new = x5 * d / (d + y5);
    int y5_new = y5 * d / (d + y5);
    int x6_new = x6 * d / (d + y6);
    int y6_new = y6 * d / (d + y6);
    int x7_new = x7 * d / (d + y7);
    int y7_new = y7 * d / (d + y7);
    int x8_new = x8 * d / (d + y8);
    int y8_new = y8 * d / (d + y8);
    draw3dObject(x1_new, y1_new, x2_new, y2_new, x3_new, y3_new, x4_new, y4_new, x5_new, y5_new, x6_new, y6_new, x7_new, y7_new, x8_new, y8_new);
}
int main()
{
    ...
    prospectiveProjection(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, x7, y7, x8, y8, d);
    delay(5000);
    closegraph();
    return 0;
}
